 ...
 */
function soshake_fb_output_og_protocol( $property, $content ) {
	...
	if ( is_array( $content ) ) {
		foreach( $content as $structured_property => $content_value ) {
			...
			if ( ! is_string( $structured_property ) || $structured_property === 'url' )
				soshake_fb_output_og_protocol( $property, $content_value );
			else
				soshake_fb_output_og_protocol( $property . ':' . $structured_property, $content_value );
		}
	}
	else {
 ...
 * @since 1.0
 */
function soshake_fb_add_og_protocol() {
	global $post;

	$meta_tags = array(
		'http://ogp.me/ns#locale' => soshake_fb_get_locale(),
 ...
 * @since 1.0
 */
function soshake_fb_get_locale() {
...
add_action( 'wp_head', 'soshake_fb_add_og_protocol' );